...
const TrancespaceFortress3Form: FC<Props> = observer((props: Props): JSX.Element => {
  const { accountStore, menuStore, mapStore } = useStore();
  const [bankName, setBankName] = useState(props.bankName);
  const [authorID, setAuthorID] = useState(mapProps.get(Maps.TRANCESPACE_FORTRESS_3).authorID);
  const mapTitle: string = mapProps.get(Maps.TRANCESPACE_FORTRESS_3).title;
  ...
  const callbacks = {
    ...
    onResetClick: useCallback((): void => {
      ...
      setAuthorID(mapProps.get(Maps.TRANCESPACE_FORTRESS_3).authorID);
  ...
  return (
    ...
    >
      <div style={{ color: 'white', fontSize: '24px' }}>Work in progress...</div>
    </Editor>
  );
});

export default React.memo(TrancespaceFortress3Form);